...
\hypersetup{
  ...
  pdfauthor={Michael Dumelle; Erin E. Peterson},
  colorlinks=true,
  linkcolor={blue},
  filecolor={Maroon},
  citecolor={Blue},
...
(complete seagrass cover). Figure~\ref{fig-cover_fy_mb} shows total
...
of the Bay. Note that 2022-2023 (post-flood) had the largest number of
seagrass cover samples, as well as the only samples collected in the
middle of the Bay.
...
are used to represent geographic features, proximity to rivers, and
...
structure, 2) visualize spatial and temporal patterns in the the data,
3) identify outliers/errors, 4) examine correlations between potential
...
tidal range, turbidity, temperature, and time since flood given a goal
was to study the extent to which these variables were related to
seagrass cover. We removed other variables showing no meaningful
association with seagrass cover or where the relationship lacked
biological plausibility. Salinity metrics had little usable pre flood
data, which did not show strong relationships with seagrass cover.
Geographic variables exhibited less collinearity than water quality
variables; however, we removed the distance to the nearest river outlet
and the nearest river variables due to weak associations with seagrass
cover after accounting for other variables in the model. Growing season
was excluded because a simple monthly proxy inadequately captures
seasonal seagrass growth dynamics.
...
Out-of-sample predictions were then used to calculate the mean bias
(MBias) and root-mean-square-prediction error (RMSPE) for each model.
...
suggested that linear effects of the model variables were generally
reasonable. Given these results and the ease of interpretability of the
spatial linear model, we chose to use a spatial linear modeling
framework for the Moreton Bay seagrass cover model.
...
relative to root-mean-squared-prediction error (RMSPE). For LM, SPLM,
SPGLM-Beta, SPGLM-Gamma, leave-one-out cross validation was used. For
RF, out of bag validation was used.}
...
\texttt{spmodel}. Parameters were estimated using restricted maximum
likelihood. Models were compared primarily using leave-one-out cross
validation, though likelihood-based approaches like AIC or BIC could
also be used (when the fixed effects change across models, AIC and BIC
are directly comparable only when the estimation method is maximum
likelihood). Ultimately, after considering a handful of models alongside
fit statistics (e.g., leave-one-out cross validation error) and the
research questions of interest, we settled on a final model, which is
described next.
...
these baselines. For the numeric variables, all had large test
statistics and small \(p\)-values (\(p\)-value \(<\) 0.01). While we can
assess the overall significance of the numeric variables
Table~\ref{tbl-mb-fixed}, we must perform an analysis of variance
(ANOVA) to assess the overall significance of variables with many
levels. Table~\ref{tbl-mb-covariates} shows these ANOVA results for the
sediment type and tidal range variables, which had large test statistics
and small \(p\)-values (\(p\)-value \(<\) 0.01). Together, these results
...
intertidal depth range and sand sediment types.}
...
seagrass cover is recovering effectively across all tidal ranges. When
we examine recovery for the whole of Moreton Bay
(Figure~\ref{fig-bk_mb_all}), the model suggests that seagrass cover has
improved since the 2022 flood.
...
resilience, and then characterizing the most important drivers of that
resilience.
...
covariates so we accounted for them using B-spline basis functions
...
(ANOVA) to assess the overall significance the remaining variables
(which have at least two terms). Table~\ref{tbl-mb-res-anova} shows
these ANOVA results; here are some relevant takeaways from both tables:
...
and time. Sixth, the location of sample sites was not generated randomly
and could be subject to conscious or unconscious selection bias.
