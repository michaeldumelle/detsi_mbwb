...
* Mean Flood Turbidity/(Years Since Flood)$^2$: An one-unit increase in mean turbidity during the flood is associated with a decrease in average total cover by 2.05% points/(Years Since Flood)$^2$ (@fig-mb_turb_90d_fld).
...
#| fig-cap: "Residuals for reslience analysis in Moreton Bay by financial year."
...
mb_tc_splm_mod_res %>%
  ...
  mutate(effects = c("Dominant Species", "Species Richness", "Bathymetry Mean Depth",
         "Bathymetry Mean Curvature", "Gastopod Habitat Total Area", 
         ...
         "Distance to Gastopod Habitat", "Distance to Mangrove Habitat", 
...
* Bathymetry Mean Curvature: Sites with less extreme slopes (between zero and one) tended to have increasing resilience with slope, while sites with more extreme slopes (greater than one) tended to have decreasing resilience with slope.
...
* Bathymetry mean distance was fourth-most important, with a general decreasing trend with decreasing depth [@fig-rf_bathy], similar to the spatial linear model.
...
```{r}
#| label: fig-rf_bathy
#| fig-cap: "Random forest partial dependence for total cover as a function of bathymetry mean depth."
#| out-width: "50%"
...
pred_grid_rf <- pred_grid %>%
  st_drop_geometry() %>%
  select(bath.m.mean) %>%
  mutate(dom.spp = "ZHMU", ehmp.subzone = "Southern Banks") %>%
  slice(seq(1, n(), by = 10))
