...
Seagrass meadows are a critical component of the Moreton Bay ecosystem, providing habitat for a wide array of important marine species like dugongs and green turtles. This report presents a spatially explicit statistical analysis of total seagrass cover in Moreton Bay across three financial years (2020–2021, 2022–2023, and 2023–2024), spanning a major flood event in 2022 that caused widespread seagrass loss. Using eReefs water quality data (turbidity, salinity, and temperature) alongside field observations and geographic data, we address three management-relevant research questions: what water quality conditions trigger concern for seagrass cover, whether and where cover has recovered since the flood, and what site-level characteristics make seagrass meadows more or less resilient to flood-related disturbances.

Results from spatial, linear mixed-effects models indicate that ambient turbidity is a strong driver of total percent seagrass cover, with a one-unit increase in 90-day mean turbidity associated with a 7.18 percentage point decrease in cover; a relationship that holds across the bay but is more arguably consequential in intertidal and mixed-sediment environments where cover is naturally highest. Flood-driven turbidity also negatively affected cover, though its impact diminished with time since the flood. Bay-wide, seagrass cover showed measurable recovery between 2022–2023 and 2023–2024, but this recovery was spatially uneven: EHMP subzones including Deception Bay and South-Eastern Moreton Bay returned to pre-flood levels, while Southern Bay in particular continued to lag; Conservation zones including Conservation Park Zone and Marine National Park Zone returned to nearly pre-flood levels, while General Use Zone had the least amount of negative impact from the flood. Intertidal sites recovered more slowly than deep subtidal or shallow subtidal sites. 

A second model was used to quantify site-specific resilience to flood events. A spatial linear mixed-effects model was fit the raw residuals of the first model, with covariates representing geographic and biological characteristics that make a specific site recover more or less quickly. The results indicated that sites dominated by CS or ZMHU species, with relatively high species richness, or located in deeper waters tended to support the most resilient seagrass cover. Together, the findings of the two models can be used to inform when and where seagrass management is likely to be most effective.


# Background

Over the past few decades, various scientific studies have informed our current understanding of trends, behaviors, and flood responses of seagrass cover in Moreton Bay [@abal1996seagrass; @udy1997growth; @samper2016organic; @ovsyanikova2026spatial]  as well as Australia more widely [@kilminster2015unravelling; @gilby2018seagrass; @o2018seagrass]. Remote sensing has offered new capabilities for seagrass cover mapping [@roelfsema2009integrated], while innovative statistical approaches have offered novel methods for studying resilience at site-specific scales [@gilby2023drivers]. Our work adds to this body of literature by applying spatially explicit statistical models [@cressie1993statistics] to study total percent seagrass cover (i.e., seagrass cover) drivers, resilience, and recovery in both Moreton Bay. Specifically, we provide and contextualize answers to four specific research questions, applicable to Moreton Bay:
...
The seagrass cover observations were provided by Science Under Sail Australia (SUSA; https://www.scienceundersail.com.au/). The full dataset included over 10,000 observations across three financial years (2020-2021, 2022-2023, and 2023-2024). Water quality covariates were generated from eReefs data [@steven2019ereefs]; however, modeled turbidity and salinity data were only available for 7,657 observations. Because our research questions involved connecting water quality covariates to management actions, we focused on this subset of 7,657 observations for modeling. Of the 7,657 observations, 1,995 were collected in 2020-2021 (pre-flood), 3,329 in 2022-2023 (post-flood), and 2,333 in 2023-2024 (@tbl-n_fy_mb). 
...
Total cover is a percentage that ranges from 0 (no seagrass) to 100 (complete seagrass cover). @fig-cover_fy_mb shows total cover by financial year throughout the Bay. Total cover tends to be highest near the eastern shoreline of the Bay and lowest in the middle of the Bay. @fig-cover_fy_mb also reveals that 2022-2023 had by far the most sampling, and the only samples collected in the middle of the Bay.
...
Several water quality and geographic covariates were considered in the modelling. @tbl-wq_vars_mb summarizes the water quality covariates provided by EcoFutures, which included both ambient and flood-related turbidity, salinity, and temperature metrics. The "Variable" column indicates the water quality variable turbidity, salinity, or temperature. The "Flood" variable indicates whether the measurement corresponds to the flood period or not. The "Period" variable indicates the temporal period (in days) prior to sampling or indicates whether the variable corresponds to a flood event. The "Type" column is the type of statistic, either a mean, minimum, maximum, number of days above (or below) a cutoff during "Period", or a maximum spell (i.e., duration) above (or below) a cutoff during "Period". The "Included" variable indicates whether the variable was included as a covariate in the final spatial model. 

Exploratory analyses indicated that many of the water quality variables were strongly correlated with one another. Therefore, only a subset of variables were included in the final model. This subset included mean turbidity in the 90 days prior to sampling, mean turbidity during the flood event, and mean temperature in the 90 days prior to sampling. There was no evidence that the salinity variables were strongly related to total seagrass cover and so they were omitted from the models. We provide more detail regarding the modeling in @sec-mb_model.

```{r}
#| label: tbl-wq_vars_mb
#| tbl-cap: "Moreton Bay water quality covariates considered for modeling."
turb_vars <- tibble(
...
@tbl-geo_vars_mb summarizes the geographic variables provided by EcoFutures, which have the following structure:
...
Collinearity was less prevalent among geographic variables than water quality variables. From @tbl-geo_vars_mb, we retained the sediment type, tidal range, and years since flood variables as model covariates. The nearest river and distance to nearest outlet variables were removed, as they showed weak associations with total cover. We removed growing season from the model after determining that it is not ecologically meaningful, as a simple monthly proxy inadequately accounts for the complexities of seasonal seagrass growth dynamics. We included EHMP subzone as a random effect to account for within-subzone correlation that may impact the relationships between total cover and the remaining variables. We provide more detail regarding the statistical modeling in @sec-mb_model.
...
* 90 Day Temperature: A one-degree (C) increase in 90-day water temperature prior to sampling is associated with a decrease in average total cover by 0.88%.
* Mean Flood Turbidity/(Years Since Flood)$^2$: A one-unit increase in mean turbidity during the flood is associated with a decrease in average total cover of 2.05%/(Years Since Flood)$^2$ (@fig-mb_turb_90d_fld).
* 90 Day Turbidity: A one-unit increase in 90-day turbidity (prior to sampling) is associated with a decrease in average total cover by 7.18% (@fig-mb_turb_90d_fld).
...
#| fig-cap: "Average total seagrass percent cover as a function of 90-day mean turbidity (left) and flood turbidity (right). Estimates (and confidence intervals) were evaluated at one year post flood, at the means of all other numeric variables, and for the intertidal depth range and sand sediment type (the same trends occur for both variables among the remaining species and subzones)."
...
Together, these results suggest that elevated ambient and flood-driven turbidity may serve as useful management triggers. For instance, given the negative relationship between turbidity and total cover, managers could establish an ecologically or biologically meaningful turbidity threshold above which intervention is warranted. 

We examined model diagnostics to better understand the overall model fit. Here we discuss how to partition the model into different components of variability, infer decay behavior of spatial autocorrelation, analyze standardized residuals to assess model assumptions, and identify influential observations that have a sizable impact on model fit. 

First, we partitioned the model into different components of variability (@tbl-mb_varcomp) and found that 11.2% is attributable to the covariates (this quantity is sometimes called the Pseudo R-squared), 31.6% to the spatial random error, 7.3% to the EHMP subzone, and the remaining variability is independent random error. Second, we examined the decay behaviour of spatial autocorrelation. The estimated exponential spatial range ($\phi$) is 745.5 meters, which suggests two total seagrass cover observations are approximately (spatially) uncorrelated at a distance of $3\phi \approx 2$km, or two kilometers. Third, we analyzed the standardized model residuals, which have been decorrelated, to assess model assumptions and identified influential observations that have a sizable impact on model fit. Generally, the standardized residuals were centered around zero, though there is a slight right skew suggesting some observed total cover values are larger than expected (according to the model). Observations are referred to as "influential" if they highly influence model fit. Influence is often assessed via Cook's distance, and no observations exceeded the often-used Cook's distance threshold value of one [@cook1982residuals].
...
A tremendous benefit of using the spatial linear model is that block Kriging can be directly applied to predict the mean total seagrass cover bay-wide for any combination of covariates and geographic regions, even if these variables were not directly used in modeling. @fig-bk_mb_ehmp and @fig-bk_mb_zone show that there are several EHMP subzones and conservation zones that have recovered to pre-flood levels (e.g., Deception Bay, Marine National Park Zone), while others have not (e.g., Southern Bay, General Use Zone). While EHMP subzone was included in the model as a random effect, conservation zone type was not. Nevertheless, we can still quantify broad trends in total seagrass cover using block kriging. @fig-bk_mb_tidal shows that total seagrass cover is recovering less effectively in the intertidal range than the shallow or deep subtidal ranges, while @fig-bk_mb_all shows that total seagrass cover is recovering bay-wide since the flood. Although it has not been employed here, block Kriging can also be used for scenario modeling; for example, to characterize the impact and spatially explicit recovery of seagrass cover under future flood events and water temperature scenarios.
...
Importantly, all five approaches identified important relationships among the covariates and total cover, in the directions expected. From @tbl-mb_cv, the spatial linear model and random forest had the lowest out-of-sample root-mean-squared-prediction error, followed by the generalized linear models and then by the nonspatial linear model. Random forest partial dependence plots [@greenwell2017pdp], which characterize the marginal behavior of a variable in a random forest model, suggested linear effects of the model variables to be reasonable. Given this context, we chose the spatial linear model as the final model.
```{r}
#| label: tbl-mb_cv
